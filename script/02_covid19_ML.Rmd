...
## save the out-of-sample predictions for each model evaluated during tuning step
gridControl <- tune::control_grid(save_pred = TRUE)
...
# ## test if recipe works
# preped_rec_pca <- recipes::prep(
#   x = pca_rec,
#   training = rsample::analysis(x = folds$splits[[1]])
# )
# 
# train_baked_pca <- recipes::bake(
#   object = preped_rec_pca,
#   new_data = rsample::analysis(x = folds$splits[[1]])
# )
...
LASSO Regularized Generalized Linear Models will be used on the trimmed feature
set generated after entropy based feature selection. This should reduce the time
...
lasso_rec <- rec_vst %>% 
  recipeselectors::step_select_infgain(
    recipes::all_predictors(), outcome = "class",  
    type = "infogain", threads = !!cores, threshold = 0.99
  ) %>% 
  recipeselectors::step_select_linear(
    recipes::all_predictors(), outcome = "class", engine = "glmnet",
    mixture = 1, ## implies lasso
    threshold = 0.999,
    penalty = tune()
  )

# ## test if recipe works
# preped_rec_lasso <- recipes::prep(
#   x = lasso_rec,
#   training = rsample::analysis(x = folds$splits[[1]])
# )
# 
# train_baked_lasso <- recipes::bake(
#   object = preped_rec_lasso,
#   new_data = rsample::analysis(x = folds$splits[[1]])
# )
...
rf_lasso_grid <- dials::grid_regular(
  dials::mtry(range = c(1L, 10L)),
  dials::min_n(range = c(2L, 40L)),
  dials::penalty(),
  levels = 5
...
rf_pca_best <- tune::select_best(rf_pca_res, metric = "roc_auc")

## calculate and save AUC data for future use
rf_pca_auc <- tune::collect_predictions(x = rf_pca_res, parameters = rf_pca_best) %>% 
  yardstick::roc_curve(class, .pred_positive) %>% 
  dplyr::mutate(model = "PCA + Random Forest")

readr::write_tsv(
  x = rf_pca_auc, file = here::here("data/results", "rf_pca.auc.tsv")
)

## finalize the workflow with best hyperparameters
rf_pca_final <- rf_pca_workflow %>% 
  tune::finalize_workflow(rf_pca_best)

rf_pca_holdFit <- rf_pca_final %>% 
  tune::last_fit(split = splits)

collect_metrics(rf_pca_holdFit) %>% 
  readr::write_tsv(
    file = here::here("data/results", "rf_pca.final_prediction.tsv")
  )
...
## select hyperparameters of the best performing model
rf_info_best <- tune::select_best(rf_info_res, metric = "roc_auc")

## calculate and save AUC data for future use
rf_info_auc <- tune::collect_predictions(x = rf_info_res, parameters = rf_info_best) %>% 
  yardstick::roc_curve(class, .pred_positive) %>% 
  dplyr::mutate(model = "Information Gain + Random Forest")

readr::write_tsv(
  x = rf_info_auc, file = here::here("data/results", "rf_info.auc.tsv")
)


## finalize the workflow with best hyperparameters
rf_info_final <- rf_info_workflow %>% 
  tune::finalize_workflow(rf_info_best)

rf_info_holdFit <- rf_info_final %>% 
  tune::last_fit(split = splits)

collect_metrics(rf_info_holdFit) %>% 
  readr::write_tsv(
    file = here::here("data/results", "rf_info.final_prediction.tsv")
  )
```
...
```{r}
## random forest workflow
rf_lasso_workflow <- workflows::workflow() %>% 
  workflows::add_model(rf_mod) %>% 
  workflows::add_recipe(lasso_rec)

rf_lasso_res <- rf_lasso_workflow %>% 
  tune::tune_grid(
    resamples = folds,
    grid = rf_lasso_grid,
    control = control_grid(save_pred = TRUE),
...
# tune::collect_metrics(rf_lasso_res)
# tune::show_best(rf_lasso_res)

## select hyperparameters of the best performing model
rf_lasso_best <- tune::select_best(rf_lasso_res, metric = "roc_auc")

## calculate and save AUC data for future use
rf_lasso_auc <- tune::collect_predictions(x = rf_lasso_res, parameters = rf_lasso_best) %>% 
  yardstick::roc_curve(class, .pred_positive) %>% 
  dplyr::mutate(model = "Lasso + Random Forest")

readr::write_tsv(
  x = rf_lasso_auc, file = here::here("data/results", "rf_lasso.auc.tsv")
)


## finalize the workflow with best hyperparameters
rf_lasso_final <- rf_lasso_workflow %>% 
  tune::finalize_workflow(rf_lasso_best)

rf_lasso_holdFit <- rf_lasso_final %>% 
  tune::last_fit(split = splits)

collect_metrics(rf_lasso_holdFit) %>% 
  readr::write_tsv(
    file = here::here("data/results", "rf_lasso.final_prediction.tsv")
  )
```
...
## select hyperparameters of the best performing model
svm_pca_best <- tune::select_best(svm_pca_res, metric = "roc_auc")

## calculate and save AUC data for future use
svm_pca_auc <- tune::collect_predictions(x = svm_pca_res, parameters = svm_pca_best) %>% 
  yardstick::roc_curve(class, .pred_positive) %>% 
  dplyr::mutate(model = "PCA + SVM")

readr::write_tsv(
  x = svm_pca_auc, file = here::here("data/results", "svm_pca.auc.tsv")
)

## finalize the workflow with best hyperparameters
svm_pca_final <- svm_pca_workflow %>% 
  tune::finalize_workflow(svm_pca_best)

svm_pca_holdFit <- svm_pca_final %>% 
  tune::last_fit(split = splits)

collect_metrics(svm_pca_holdFit) %>% 
  readr::write_tsv(
    file = here::here("data/results", "svm_pca.final_prediction.tsv")
  )
```
...
## select hyperparameters of the best performing model
svm_info_best <- tune::select_best(svm_info_res, metric = "roc_auc")

## calculate and save AUC data for future use
svm_info_auc <- tune::collect_predictions(x = svm_info_res, parameters = svm_info_best) %>% 
  yardstick::roc_curve(class, .pred_positive) %>% 
  dplyr::mutate(model = "Information Gain + SVM")

readr::write_tsv(
  x = svm_info_auc, file = here::here("data/results", "svm_info.auc.tsv")
)

## finalize the workflow with best hyperparameters
svm_info_final <- svm_info_workflow %>% 
  tune::finalize_workflow(svm_info_best)

svm_info_holdFit <- svm_info_final %>% 
  tune::last_fit(split = splits)

collect_metrics(svm_info_holdFit) %>% 
  readr::write_tsv(
    file = here::here("data/results", "svm_info.final_prediction.tsv")
  )
```
...
```{r}
## SVM workflow
svm_lasso_workflow <- workflows::workflow() %>% 
  workflows::add_model(rf_mod) %>% 
  workflows::add_recipe(lasso_rec)

svm_lasso_res <- svm_lasso_workflow %>% 
  tune::tune_grid(
    resamples = folds,
    grid = svm_lasso_grid,
    control = control_grid(save_pred = TRUE),
...
# tune::collect_metrics(svm_lasso_res)
# tune::show_best(svm_lasso_res)

## select hyperparameters of the best performing model
svm_lasso_best <- tune::select_best(svm_lasso_res, metric = "roc_auc")

## calculate and save AUC data for future use
svm_lasso_auc <- tune::collect_predictions(x = svm_lasso_res, parameters = svm_lasso_best) %>% 
  yardstick::roc_curve(class, .pred_positive) %>% 
  dplyr::mutate(model = "Lasso + SVM")

readr::write_tsv(
  x = svm_lasso_auc, file = here::here("data/results", "svm_lasso.auc.tsv")
)


## finalize the workflow with best hyperparameters
svm_lasso_final <- svm_lasso_workflow %>% 
  tune::finalize_workflow(svm_lasso_best)

svm_lasso_holdFit <- svm_lasso_final %>% 
  tune::last_fit(split = splits)

collect_metrics(svm_lasso_holdFit) %>% 
  readr::write_tsv(
    file = here::here("data/results", "svm_lasso.final_prediction.tsv")
  )
```
